...
def nearest_carPark(postal, main_df, k=10):
    postal_long = ""
    postal_lat = ""
    ...
    for i in range(len(main_df)):
        check_long = main_df.iloc[i]["longitude"]
        check_lat = main_df.iloc[i]["latitude"]
        distLst.append( vincenty( [float(postal_long),float(postal_lat)] , [check_long,check_lat]) )    
        
    main_df["dist"] = distLst
    sorted_df = main_df.sort_values(by=['dist'], ascending=True)
    nearest_df = sorted_df.iloc[0:k]
...
def df_to_dict(input_df):
    # Convert each row of input df to a dictioanry, and ouputs a list of dictionaries
    temp = input_df.to_dict(orient='index') 
